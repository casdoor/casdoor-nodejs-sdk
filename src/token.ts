...
export class TokenSDK {
    ...
    public async getTokens(p: number, pageSize: number) {
        ...
        return (await this.request.get('/get-tokens', {
            params: {
                p: String(p), pageSize: String(pageSize), owner: this.config.orgName, clientId: this.config.clientId, clientSecret: this.config.clientSecret,
            },
    ...
    public async modifyToken(method: string, token: Token) {
        ...
        return (await this.request.post(url, {tokenInfo}, {
            params: {
                id: `${token.owner}/${token.name}`, clientId: this.config.clientId, clientSecret: this.config.clientSecret,
            },
