...
export class ApplicationSDK {
  ...
  public async isGrantTypeAllowedInApp(
    grant_type = 'password',
    app_name: string,
  ) {
    if (!this.request) {
    ...
    try {
      if (!app_name && this.config.appName) {
        app_name = this.config.appName
      }
      const application = await this.getApplication(app_name)

      if (!application) {
        throw new Error(
          `Application ${this.config.appName} not found in organization ${this.config.orgName}`,
        )
      }
      return (
        Array.isArray(application.data.data.grantTypes) &&
        application.data.data.grantTypes.includes(grant_type)
      )
    } catch (error) {
      console.error('Error verificando grant type:', error)
      throw error
    }
  }
