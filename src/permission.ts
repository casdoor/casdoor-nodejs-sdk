...
export class PermissionSDK {
    ...
    public async modifyPermission(method: string, permission: Permission) {
        ...
        return (await this.request.post(url, {permissionInfo}, {
            params: {
                id: `${permission.owner}/${permission.name}`, clientId: this.config.clientId, clientSecret: this.config.clientSecret,
            },
